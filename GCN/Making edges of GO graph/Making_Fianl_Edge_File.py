...
all_children, alt_id = read_go_children("go_cafa3.obo")
check = []

go_ic = {}
with open('GOA_bpo_IC.txt') as lines:
    for line in lines:
        data = line.split('\t')
...
go_cnt = {}
with open("GOA_bpo_cnt.txt") as lines:
    for line in lines:
        data = line.split('\t')
        cnt = float(data[1].replace('\n', ''))
        go_cnt[data[0]] = cnt

f1 = open("BPO/all_go_bpo_cnt.tsv", "w")
with open("BPO/all_go_bpo_edge.tsv") as read_in:
    for line in read_in:
