...
def load_edge_list(path, symmetrize=False, valid_indexes = None):
    df = pd.read_csv(path, header=None, sep='\t', engine='c')
    df.dropna(inplace=True)

    if valid_indexes is not None:
        mask = df[0].isin(valid_indexes)
        df = df[mask]
...
def build_graph(FOLDER, SMALL_TYPE, valid_indexes = None):
    data_dir = Path(f"C:/Users/T580/Desktop/ELTE/#SZAKDOLGOZAT/Effective-GCN-based-Hierarchical-Multi-label-classification-for-Protein-Function-Prediction/dataset/{FOLDER}")
    edge_list_dir = data_dir / f"all_go_{SMALL_TYPE}o_IC.tsv"
    node_list_dir = data_dir / f"all_go_{SMALL_TYPE}o_only_num.tsv"
    one_hot_dir = data_dir / f"all_go_{SMALL_TYPE}o_parents_only_num.tsv"

    idx, objects, IC = load_edge_list(edge_list_dir,False, valid_indexes)
    idx_2d = idx
    ...
    if valid_indexes is not None:
        filtered_list = []
        for ids in go_id:
            if f"GO:{ids}" in valid_indexes:
                filtered_list.append(ids)
        tmp = ['GO:' + str(element) for element in filtered_list]
        go_id = filtered_list

    # making dictionary
    idx_map = dict(zip(go_id, idx))    # 10236
    label_map = dict(zip(tmp, idx))    # 10236
    ldx_map_ivs = dict(zip(idx, go_id))# 10236
    label_map_ivs = dict(zip(idx, tmp))# 10236

    # build symmetric adjacency matrix
    adj = build_adj(idx_2d, IC, idx_map) # 3 ugyanolyan méretű
