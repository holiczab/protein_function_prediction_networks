...
def train_model(args):
    ...
    # Initialize model
    model = Net(len(reindexed_label_map), args).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            # Forward pass
            student_logits = model(input, one_hot_node, adj)
            student_probs = torch.sigmoid(student_logits)

            # Compute standard BCELoss
            loss = criterion(student_probs, target)

            # Knowledge Distillation
            if teacher_model is not None:
                with torch.no_grad():
                    teacher_logits = teacher_model(input, one_hot_node, adj)
                soft_teacher_probs = F.softmax(teacher_logits / T, dim=1)
                soft_student_probs = F.softmax(student_logits / T, dim=1)
                distillation_loss = nn.KLDivLoss(reduction='batchmean')(soft_student_probs.log(), soft_teacher_probs)
